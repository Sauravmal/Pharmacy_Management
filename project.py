...
mydb = sq.connect(host="localhost", user="root", passwd="Saurav@2002", database="pharmacy")
mycur = mydb.cursor()
...
def admin():
    def window_1():
        ...
        def updatedb():
            stmt = "update medicines_info set Med_name=%s,Qty=%s,MRP=%s,Mfg=%s,Exp=%s,Purpose=%s where Med_code=%s"
        ...
        def selectdb():
            ...
            canvas = Canvas(n3, bg="white")
            scrollbar = Scrollbar(n3, orient=VERTICAL, command=canvas.yview)
            ...
            j = 0
            for i in result:
                s += " ( " + str(i[0]) + " )  " + str(i[1]) + " - " + str(i[2]) + " -  Rs." + str(i[3]) + " - " + str(
                    i[4]) + " - " + str(i[5]) + " - " + str(i[6])
                l_3 = Label(n3, text=s,
                            font="consolas 14",
                            bg="white")
            ...
            for idx, med in enumerate(result):
                line = f"( {med[0]} )  {med[1]} - {med[2]} - Rs.{med[3]} - {med[4]} - {med[5]} - {med[6]}"
                lbl = Label(scrollable_frame, text=line, font="consolas 14", bg="white", anchor="w", justify=LEFT)
